...
const OPD = () => {
  ...
  const updateRow = (index: number, newData: Partial<OPDItem>) => {
    ...
    const medicalScheme =
      newData.MedicalScheme || updatedData[index].MedicalScheme
  ...
  const handleSaveRow = async (index: number) => {
    try {
      ...
      if (
        ...
        !item.LastName ||
        !item.Treatment ||
        (!item.Amount && !item.MedicalScheme)
      ) {
  ...
  const verifyAdminPassword = async (password: string): Promise<boolean> => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/AdminPassword`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
                ...
                <tbody>
                  {opd.map((row, index) => (
                    <tr key={index} className='border-t border-gray-200'>
                      ...
                      <td className='px-4 py-2'>
                        <button
                          className='bg-red-500 text-white hover:bg-red-700 hover:text-white focus:outline-none px-4 py-2 rounded mr-2'
